...
**Migration serialization.** Before starting any _new_ task (step 4) that would add a Drizzle migration, check whether another currently-open `agent/` PR already adds one. If so, do not add a second one in parallel — pick a non-migration task instead, or wait for the in-flight one to merge first. Two branches each independently generating a migration against the same `main` state is exactly how PR #179 ended up with a broken snapshot chain in the first place (see `docs/execution/decisions/` for the incident this rule codifies); 3a's regenerate step above is the recovery path for when this has already happened, not a substitute for avoiding it.
