...
For every open PR on an `agent/` branch, find its actual last Agent 90 verdict — not just the last entry in the reviews array, which can be a different reviewer, a dismissed review, or a stale one:

```
gh pr view <n> --json mergeStateStatus,reviews --jq '{mergeStateStatus, lastAgent90Review: ([.reviews[] | select(.body | startswith("AGENT-90-REVIEW:"))][-1].body // "")}'
```

If that verdict says `PASS`/approved with 0 BLOCKER and 0 HIGH, but `mergeStateStatus` is not `CLEAN`/current (stale against `main`, e.g. because another PR merged since its last CI run), that PR just needs a rebase, not new work:

1. `git fetch origin`, check out the branch, merge or rebase `origin/main` into it.
2. If the only conflict is a mechanical one this repo is known to produce — two PRs claiming the same Drizzle migration number/snapshot filename (check `packages/database/drizzle/*.sql` and `drizzle/meta/_journal.json` on `main` for what's actually taken) — **do not hand-rename the file or hand-edit the journal tag.** A Drizzle snapshot carries a `prevId`/`tables` lineage, not just a filename; a bare rename leaves `prevId` pointing at the wrong predecessor and the `tables` map missing every sibling table that landed on `main` since this branch's migration was generated, which silently corrupts the _next_ migration anyone generates from this chain. Instead: delete this branch's own stale migration `.sql` file, its `meta/<N>_snapshot.json`, and its entry in `meta/_journal.json` (only its own — never another PR's). When the merge itself reports an add/add conflict on that same snapshot filename, resolving it is easy to get backwards: `git merge origin/main` makes `origin/main`'s version "theirs" and your branch's version "ours" — you want to keep theirs (`git checkout --theirs <path>`) and discard yours, since `main`'s copy is the one whose migration slot you're not colliding into. Verify after resolving that the kept file's `id` matches what's actually on `main` before regenerating, not just that the merge reports clean. Then regenerate with drizzle-kit against the now-rebased `main` state exactly the way a brand-new migration is created elsewhere in this codebase (`DATABASE_URL=<a reachable disposable Postgres> pnpm --filter @fitness-os/database db:generate`, run from a workspace where `packages/schemas`/`packages/domain` are already built). Rename the freshly generated file to this repo's `NNNN_prdXX_description.sql` convention (matching the original migration's description) and fix the journal `tag` to match, exactly as when authoring any new migration — but let drizzle-kit itself compute `prevId` and the `tables` map; never hand-write those fields.
3. Re-run the full quality gate suite from step 8 (lint, format:check, build:packages, typecheck, test, build) against the rebased/regenerated state before pushing — a rebase or a regenerated migration is exactly the kind of change that can pass gates individually and still break when combined. Do not push on a hunch that it's still green.
4. Push the rebased branch. Do not open a new PR for this — the existing PR updates automatically.
...
**Migration serialization.** Before starting any _new_ task (step 4) that would add a Drizzle migration, check whether another currently-open `agent/` PR already adds one. If so, do not add a second one in parallel — pick a non-migration task instead, or wait for the in-flight one to merge first. Two branches each independently generating a migration against the same `main` state is exactly how PR #179 ended up with a broken snapshot chain in the first place (see GitHub issue #181 for the incident this rule codifies); 3a's regenerate step above is the recovery path for when this has already happened, not a substitute for avoiding it.
