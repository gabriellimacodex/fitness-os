...
1. Continue remaining PRD 07/21 synthetic/disposable mechanics — the next
   smallest PRD 21 H4 composition is to inject the landed PG lifecycle binding
   verifier into the API/bootstrap path and then replace caller-trusted retention
   execution booleans with the landed persisted-preview authorization plus
   trusted current inventory/processor evidence. H5 still needs runtime readiness
   binding for the reviewed exact-once record-family inventory, including
   fail-closed missing/duplicate/mismatched family or unreviewed-exception
   outcomes. Full coordinator semantics remain open. Do not invent legal
...
4. **PRD 21** — continue Option A from current `main` (integrated through #227,
   ...
   - H4 residuals remain (API/bootstrap composition of the landed PG lifecycle
     verifier, route-level persisted-preview/current-runtime authorization,
     execution transition, and full coordinator semantics across the landed
     plan/step/resume/completion/lifecycle pieces);
