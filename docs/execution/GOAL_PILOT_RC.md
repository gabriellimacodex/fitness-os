...
1. Continue remaining PRD 07/21 synthetic/disposable mechanics — #230 landed an
   ...
   evidence. Retention-rule injection and exact inventory/runtime injection into
   that helper, server-bootstrap adoption, and fail-closed reviewed-exception
   outcomes remain open. Full
...
4. **PRD 21** — continue Option A from current `main` (integrated through #256,
   ...
   - H5 contract coverage and PG expected-inventory/runtime binding landed;
     #253 establishes the PG platform helper, but retention-rule injection, exact
     inventory/runtime injection, bootstrap adoption, and reviewed exception
