...
- Last refreshed: 2026-09-02 (integrated delivery refresh through #236)
- Progress snapshot head: `f6de54e` (PR #236 merged; see Progress)
...
| PRD 21               | `IN_PROGRESS` — Option A synthetic through #236. H3 remains closed. H4 now includes trusted intake, server-derived processor plans, guarded completion/resume, PG step/lifecycle/retention-preview/retention-rule persistence, rule-aware preview routing, post-persistence PG lifecycle-proof lookup, an independent read-only execution-receipt source used for fail-closed pre-append verification (#230), trusted persisted-preview/current-runtime authorization (#232), exact-once persisted-preview consumption (#234), and server-authoritative processor-step plan/timestamp/transition identity with exact immutable replay matching (#236). No destructive processor action is enabled; processor outcome remains caller-supplied synthetic evidence and full coordinator semantics remain open. Privacy readiness evidence includes the governance-lifecycle table; migration recovery evidence landed in #209. H5 exact-once record-family contract coverage remains landed while runtime readiness and reviewed-exception coverage remain open. Gate A `PENDING`; production `BLOCKED` by `LEGAL_PRIVACY`; H6 remains open. See `gates/PRD_21_GATE_A.md`. |
...
1. Continue remaining PRD 07/21 synthetic/disposable mechanics — #230 landed an
   ...
   #234 added exact-once preview consumption, and #236 made processor-step plan,
   timestamp, replay, and transition identity server-authoritative. The next
   smallest PRD 21 H4 slice is binding the caller-supplied synthetic processor
   outcome to an independent trusted execution receipt before append/advance,
   while preserving fail-closed synthetic-only behavior and enabling no
   destructive processor action. H5 still needs runtime readiness
