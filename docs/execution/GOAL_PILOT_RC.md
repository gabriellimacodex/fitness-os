...
- Last refreshed: 2026-09-03 (integrated delivery refresh through #263)
- Progress snapshot head: `8bb9def` (PR #263 merged; see Progress)
...
| PRD 21               | `IN_PROGRESS` — Option A synthetic through #263. H3 remains closed. H4 includes trusted intake, server-derived processor plans, guarded completion/resume, PG step/lifecycle/retention-preview/retention-rule persistence, rule-aware preview routing, post-persistence PG lifecycle-proof lookup, trusted persisted-preview/current-runtime authorization (#232), exact-once persisted-preview consumption (#234), server-authoritative processor-step plan/timestamp/transition identity with exact immutable replay matching (#236), an independent runtime-validated processor execution-receipt source (#238), a coordinator-owned non-destructive synthetic execute→receipt→step path (#240), durable pre-execution operation ownership with exact completed replay plus fail-closed restart hold (#242), and trusted independent-receipt reconciliation without handler re-execution, including concurrent convergence (#263). #252 adds retention rules to the PG persistence bundle, #253 adds an environment-gated PG platform helper, #256 binds recovery readiness to real trigger evidence, and #261 injects the bundle's real retention-rule repository into that helper. No destructive processor action is enabled; server-bootstrap adoption, exact inventory/runtime injection into the platform helper, and full coordinator semantics remain open. Gate A `PENDING`; production `BLOCKED` by `LEGAL_PRIVACY`; H6 remains open. See `gates/PRD_21_GATE_A.md`. |
...
`0002`–`0006` + `0011`–`0012` + `0014`–`0015` + `0017`–`0023` + `0025` privacy;
...
1. Continue remaining PRD 07/21 synthetic/disposable mechanics — #230 landed an
   ...
   work without re-execution. #263 resolves that hold from one exact independent
   receipt through a dedicated PG transition, including concurrent convergence,
   without invoking the handler again. It does not expose caller authority,
   ...
   adds an env-gated PG platform helper, #256 binds recovery to trigger
   evidence, and #261 injects the bundle's real retention-rule repository into
   that helper. The next smallest composition slice is exact reviewed
   inventory/runtime injection into the env-gated helper, while keeping server
   bootstrap disabled. Server-bootstrap adoption,
   fail-closed reviewed-exception outcomes, and full coordinator semantics
   remain open. Do not invent legal exception policy, Gate A PASS, or registry
   `COMPLETED` under active stops.
...
4. **PRD 21** — continue Option A from current `main` (integrated through #263,
   ...
   - #230 adds an independent read-only execution/coordinator receipt source for
     ...
     execute→receipt→step composition and restart-safe durable ownership; #263
     adds trusted reconciliation without handler re-execution or concurrency
     drift, while full coordinator semantics remain open;
   - H5 contract coverage and PG expected-inventory/runtime binding landed;
     #253 establishes the PG platform helper and #261 adds retention-rule
     injection, but exact inventory/runtime injection, bootstrap adoption, and
     reviewed exception semantics remain open;
