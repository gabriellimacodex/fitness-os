...
- Last refreshed: 2026-09-03 (integrated delivery refresh through #248)
- Progress snapshot head: `452e326` (PR #248 merged; see Progress)
...
| PRD 07               | `IN_PROGRESS` — synthetic/disposable persistence and TD 007 ports are composed; #192 and the #227 integration add coach-bootstrap/full issue→claim/revoke/resume/abandon PG evidence, transition-sink composition, canonical-input/idempotency guards, trusted-time attempt evaluation, claim-secret throttling, and student/coach onboarding screens. The #227 correction makes throttled responses non-oracular while preserving safe retry replay/mismatch semantics. #246 binds the four PG repository readiness components to real schema evidence, #247 binds clock/id/secret components to real self-tests, and #248 enables the PG readiness probe to evaluate caller-supplied mechanism instances. API/runtime composition of that combined probe remains open. Real-user remains under `LEGAL_PRIVACY`; Gate A is not claimed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| PRD 21               | `IN_PROGRESS` — Option A synthetic through #245. H3 remains closed. H4 now includes trusted intake, server-derived processor plans, guarded completion/resume, PG step/lifecycle/retention-preview/retention-rule persistence, rule-aware preview routing, post-persistence PG lifecycle-proof lookup, trusted persisted-preview/current-runtime authorization (#232), exact-once persisted-preview consumption (#234), server-authoritative processor-step plan/timestamp/transition identity with exact immutable replay matching (#236), an independent runtime-validated processor execution-receipt source that owns the recorded outcome (#238), a coordinator-owned non-destructive synthetic execute→receipt→step path (#240), and durable pre-execution operation ownership with exact completed replay plus fail-closed restart reconciliation (#242). No destructive processor action is enabled; trusted resolution of `reconciliation_required` and full coordinator semantics remain open. Privacy readiness evidence now binds governance-lifecycle, expected-inventory/runtime coverage (#244), and the audit-sink table (#245); migration recovery evidence landed in #209. API composition of those real options and reviewed-exception coverage remain open. Gate A `PENDING`; production `BLOCKED` by `LEGAL_PRIVACY`; H6 remains open. See `gates/PRD_21_GATE_A.md`. |
...
1. Continue remaining PRD 07/21 synthetic/disposable mechanics — #230 landed an
   ...
   inventory/runtime coverage inside the PG probe and #245 binds audit-sink table
   evidence; API composition and fail-closed reviewed-exception outcomes remain open. Full
...
4. **PRD 21** — continue Option A from current `main` (integrated through #245,
   ...
   - H5 contract coverage and PG expected-inventory/runtime binding landed;
     API composition and reviewed exception semantics remain open; H6 remains open;
