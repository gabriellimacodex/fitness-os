...
- Progress snapshot head: `06a92e5` (PR #132 merged; see Progress)
...
| PRD 07               | `IN_PROGRESS` — synthetic/disposable persistence + TD 007 ports composed in API through #132 (IdentitySession, PrincipalBinding/Reference, ClaimRepository, TransitionSink full lifecycle + create→claim and issue→revoke `list()` chain tests, Id/Secret factories, SecretVerifier, TrustedClock). Real-user still `LEGAL_PRIVACY`. Tip `06a92e5`. |
| PRD 21               | `IN_PROGRESS` — Option A synthetic seams through #131 (clock/ids, inventory-coverage + expected/runtime port injects, GET expected/runtime, triad + mismatch via ports, contracts freezes). Gate A status `PENDING` — **not** PASS; production `BLOCKED` by `LEGAL_PRIVACY`. See `gates/PRD_21_GATE_A.md`.                                          |
