...
- Progress snapshot head: `aa6775b` (PR #135 merged; see Progress)
...
| PRD 07               | `IN_PROGRESS` — synthetic/disposable persistence + TD 007 ports composed in API through #134 (IdentitySession, PrincipalBinding/Reference, ClaimRepository, TransitionSink full lifecycle + create→claim / issue→revoke / resume→abandon `list()` chain tests, Id/Secret factories, SecretVerifier, TrustedClock). Real-user still `LEGAL_PRIVACY`. Tip `aa6775b`. |
| PRD 21               | `IN_PROGRESS` — Option A synthetic seams through #135 (clock/ids, inventory-coverage + expected/runtime port injects, GET expected/runtime, triad + empty/undeclared mismatch via ports, contracts freezes). Gate A status `PENDING` — **not** PASS; production `BLOCKED` by `LEGAL_PRIVACY`. See `gates/PRD_21_GATE_A.md`.                                        |
