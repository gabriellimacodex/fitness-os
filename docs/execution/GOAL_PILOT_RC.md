...
- Last refreshed: 2026-09-02 (integrated delivery refresh through #227)
- Progress snapshot head: `68e973c` (PR #227 merged; see Progress)
...
| PRD 07               | `IN_PROGRESS` — synthetic/disposable persistence and TD 007 ports are composed; #192 and the #227 integration add coach-bootstrap/full issue→claim/revoke/resume/abandon PG evidence, transition-sink composition, canonical-input/idempotency guards, trusted-time attempt evaluation, claim-secret throttling, and student/coach onboarding screens. The #227 correction makes throttled responses non-oracular while preserving safe retry replay/mismatch semantics. Real-user remains under `LEGAL_PRIVACY`; Gate A is not claimed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| PRD 21               | `IN_PROGRESS` — Option A synthetic through #227. H3 remains closed. H4 now includes trusted intake, processor plans/steps, guarded completion/resume, PG step/lifecycle/retention-preview/retention-rule persistence, rule-aware preview routing, post-persistence PG lifecycle-proof lookup, and persisted-preview authorization that independently checks trusted current inventory/processor digests. A trusted execution/coordinator-backed verifier (or explicitly separate evidence source) is still required before the route appends to its lifecycle ledger; the same target ledger cannot serve as its own pre-append authority. Route-level persisted-preview execution and full coordinator semantics also remain open. Privacy readiness evidence includes the governance-lifecycle table; migration recovery evidence landed in #209. H5 exact-once record-family contract coverage remains landed while runtime readiness and reviewed-exception coverage remain open. Gate A `PENDING`; production `BLOCKED` by `LEGAL_PRIVACY`; H6 remains open. See `gates/PRD_21_GATE_A.md`. |
...
`0002`–`0006` + `0011`–`0012` + `0014`–`0015` + `0017`–`0019` privacy;
...
1. Continue remaining PRD 07/21 synthetic/disposable mechanics — the next
   smallest PRD 21 H4 slice is a trusted execution/coordinator-backed lifecycle
   binding verifier (or an explicitly separate evidence source), preserving the
   PG lifecycle ledger as the append target rather than making it its own
   pre-append authority. Then replace caller-trusted retention execution booleans
   with the landed persisted-preview authorization plus trusted current
   inventory/processor evidence. H5 still needs runtime readiness binding for the
   reviewed exact-once record-family inventory, including fail-closed
   missing/duplicate/mismatched family or unreviewed-exception outcomes. Full
...
3. **PRD 07** — continue synthetic/disposable composition from the current
   landed state through #227 (PG end-to-end evidence, transition sink,
...
4. **PRD 21** — continue Option A from current `main` (integrated through #227,
   ...
   - H4 residuals remain (a trusted execution/coordinator verifier or separate
     evidence source for pre-append authorization; the landed PG verifier is only
     a post-persistence lookup against that target ledger), plus route-level
     persisted-preview/current-runtime authorization, execution transition, and
