...
| Privacy PG platform helper                | `#253` (env-gated persistence/lifecycle-verifier/readiness composition; omits retention rules; bootstrap and exact inventory/runtime injection remain open)                         |
...
- Privacy DB readiness now replaces migration/repository/governance-lifecycle
  ...
  helper, but retention-rule injection, exact inventory/runtime injection,
...
- H5 contract coverage now rejects missing or duplicate mappings and the reviewed
  ...
  helper landed in #253; retention-rule and exact inventory/runtime injection
  into that helper, lifecycle-handler coverage, and independently reviewed
  exception semantics remain open; H5 is not closed.
