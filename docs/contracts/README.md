...
| Onboarding attempt timeout evaluation | `evaluateAttemptTimeout` in `packages/domain/src/onboarding/attempt.ts` | Domain onboarding ports | Unit tests covering absolute/inactivity boundaries; not yet wired into attempt creation/resume routes, `AttemptDetail`, or persisted bounds | Active |
