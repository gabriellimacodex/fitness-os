...
| Synthetic processor-step record (test seam) | `privacySyntheticProcessorStepRecordRequestSchema` / response in `privacy-governance.ts` | Synthetic API only — server derives the pinned expected plan, binds request correlation, and assigns the trusted timestamp before `recordProcessorStepAndAdvanceRequest` | Disposable `allowSyntheticPrivacy` processor-step-record tests; proves fail-closed plan/binding checks and partial-failure resume/completion; processor outcome remains caller-supplied synthetic evidence | Frozen |
