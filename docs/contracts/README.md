...
| Governance lifecycle proof (disposable PG) | `privacy_governance_lifecycle_proof` table + repository in `packages/database` (migration `0015_prd21_privacy_governance_lifecycle_proof`) | Synthetic governance-lifecycle proof persistence | Disposable PG tests; append-only, keyed by `operationId`; wired into `apps/api`'s `createPrivacyPgPersistence` bundle (`governanceLifecycle`); still not called by any processor or API route | Active |
