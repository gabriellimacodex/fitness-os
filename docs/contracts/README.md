...
| Onboarding transition (disposable PG)     | `onboarding_transition` table + `createPostgresOnboardingTransitionSink` in `packages/database`                       | Append-only by application code only — no DB-level guard trigger yet (Agent 90 R1, deferred with the other onboarding history tables) | Disposable PG integration tests; not yet wired into `apps/api` | Active |
