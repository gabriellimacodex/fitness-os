...
| Synthetic governance-lifecycle record (test seam) | `privacySyntheticGovernanceLifecycleRecordRequestSchema` / response in `privacy-governance.ts` | Synthetic API only — HTTP seam for `PrivacyGovernanceLifecycleLedger.append` | Disposable `allowSyntheticPrivacy` governance-lifecycle-record tests; `synthetic`/`recordedAt` are always server-derived, never client-supplied; append-once per `operationId`, replay returns the stored proof as `conflict` | Frozen |
...
| Governance lifecycle proof (disposable PG) | `privacy_governance_lifecycle_proof` table + repository in `packages/database` (migration `0015_prd21_privacy_governance_lifecycle_proof`) | Synthetic governance-lifecycle proof persistence | Disposable PG tests; append-only, keyed by `operationId`; wired into `apps/api`'s `createPrivacyPgPersistence` bundle (`governanceLifecycle`); reachable via `POST /v1/privacy/synthetic/governance-lifecycle-record` when composed | Active |
