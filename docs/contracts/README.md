...
| Onboarding transition (disposable PG)     | `onboarding_transition` table + `createPostgresOnboardingTransitionSink` in `packages/database`                       | Append-only onboarding transition evidence persistence                            | Disposable PG integration tests; not yet wired into `apps/api` | Active |
