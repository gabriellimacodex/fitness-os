...
| Retention preview (disposable PG) | `privacy_retention_preview` table + repository in `packages/database` (migrations `0017`, `0020`, `0021`) | Synthetic retention preview persistence plus nullable-at-rest operation binding and a status/operation consistency guard | Disposable PG tests cover atomic compare-and-set, same-operation replay, different-operation conflict, and concurrent winners; API composition remains behind `allowSyntheticPrivacy` | Active |
| Retention preview repository port | `PrivacyRetentionPreviewRepository.markExecuted` + synthetic and PostgreSQL adapters | Atomic `planned` → `executed`, first trusted `executedAt`, and one winning `operationId`; unknown digest, replay, and conflict are typed | Domain unit tests and disposable PG integration tests; no deletion/transformation side effect | Active |
