...
| Onboarding PG write-through seam | `apps/api/src/onboarding/pg-persistence.ts` behind `allowSyntheticOnboarding` | Synthetic API → disposable PG | Route write-through + hydrate tests; attempt resume/abandon transitions verified end-to-end against disposable Postgres, including idempotent replay | Active |
