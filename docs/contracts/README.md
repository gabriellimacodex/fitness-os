...
| Synthetic retention execution authorize | `privacySyntheticRetentionExecutionAuthorizeRequestSchema` / response in `privacy-governance.ts` | Synthetic API only; caller supplies only `productionMode`, `requestedSelectionDigest`, and explicit `previewTtlMs` | Disposable `allowSyntheticPrivacy` tests; legacy caller-supplied `previewExecuted`/`previewExpired`/`digestsMatch` fields are rejected | Frozen |
| Retention execution persisted-preview lookup | `resolveRetentionExecutionAuthorization` in `packages/domain`, composed by `POST /v1/privacy/synthetic/retention-execution-authorize` | Loads the exact `PrivacyRetentionPreviewRecord` from `retentionPreviews`, current inventory digest from `expectedInventory`, current descriptor digests from `processors`, and time from the trusted server clock; no state/digest authorization boolean comes from the caller | Domain unit tests cover allow, missing/mismatched/stale preview, inventory/processor changes, executed, TTL boundary, and malformed inputs; API tests cover trusted-port allow, rejection of legacy booleans, production deny, and unavailable evidence as 503; executing/marking the preview remains open | Active |
