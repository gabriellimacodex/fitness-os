...
| Onboarding PG write-through seam | `apps/api/src/onboarding/pg-persistence.ts` behind `allowSyntheticOnboarding` | Synthetic API → disposable PG | Route write-through + hydrate tests; full issue→create-attempt→policy-refresh→claim chain verified end-to-end against disposable Postgres across invitation/attempt/role-mapping/operation tables | Active |
