...
| Retention execution persisted-preview lookup | `resolveRetentionExecutionAuthorization` in `packages/domain` | Composes `authorizeRetentionExecution` with a real `PrivacyRetentionPreviewRecord` (executed/TTL/digest derived from the persisted row, never a caller-supplied boolean) and independently compares it with trusted current inventory and processor-descriptor digests; caller supplies `previewTtlMs` explicitly, no default | Domain unit tests covering allow, missing/mismatched/stale preview, inventory/processor changes, executed, TTL boundary, and fail-closed malformed-input paths; not yet wired into the `apps/api` synthetic retention-execution-authorize route, which still trusts caller-supplied booleans | Active |
