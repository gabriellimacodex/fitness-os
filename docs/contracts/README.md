...
| Synthetic data-use audit unavailable 503  | `POST /v1/privacy/synthetic/data-use-evaluate` when injected audit returns `unavailable`                              | Synthetic API → fail-closed audit path                                | Unit inject → HTTP 503 `audit_unavailable`                     | Active |
...
| Synthetic subject-request PG HTTP         | `POST /v1/privacy/synthetic/subject-request-transition` via PG `subjectRequests`                                      | Synthetic API → disposable PG subject-request                         | Integration put/applyTransition + append-only history          | Active |
