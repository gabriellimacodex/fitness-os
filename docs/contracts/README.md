...
| Processor step (disposable PG)              | `privacy_processor_step` table + `createPostgresPrivacyProcessorStepRepository` in `packages/database`, migration `0013`                     | Reuses the frozen `privacyProcessorStepReferenceSchema` (Option A) unmodified     | Disposable PG integration tests; append-only guard; not yet wired into `apps/api`                          | Active |
