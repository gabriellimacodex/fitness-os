...
| Processor execution journal | `privacyProcessorExecutionJournalRecordSchema` + `PrivacyProcessorExecutionJournal` | Durable synthetic/disposable ownership by `operationId`; exact completed replay is readable, while unfinished or ambiguous execution is held and may reach `completed` only through the dedicated reconciliation transition after exact independent-receipt verification; the handler is never automatically re-executed | PostgreSQL migrations `0023`/`0025`, guarded repository transitions, domain restart/reconciliation tests, and API PG composition; production/destructive execution remains blocked | Active |
...
| Onboarding attempt timeout evaluation | `evaluateAttemptTimeout` / `DEFAULT_ATTEMPT_TIMEOUT_BOUNDS` in `packages/domain/src/onboarding/attempt.ts` | Domain onboarding ports | Unit tests covering absolute/inactivity boundaries and fail-closed rejection of non-finite/non-positive/backward-clock input; wired into `POST /v1/onboarding/attempts` (attempt creation), which terminalizes absolute-expired attempts in the exact principal/proposed-role scope before checking uniqueness and the cap; inactivity evaluation, the resume route, `AttemptDetail` expiry surfacing, and a persisted per-attempt `lastActivityAt` bound remain unwired | Active |
...
| Synthetic processor coordinator (test seam) | `privacySyntheticProcessorCoordinateRequestSchema` / response + `PrivacyProcessorExecutionCoordinator` + `coordinateSyntheticProcessorStep` | Synthetic API only; stable pinned plan selects the next non-destructive processor/capability, a descriptor-bound execution authority exposes the outcome through a separate receipt source, and an optional durable journal reserves before execute; a held operation resolves only from one exact independent receipt through the journal's dedicated reconciliation transition | Domain/API/PG tests cover server-selected execution, exact in-process and durable restart replay, trusted held-operation reconciliation without handler re-execution, descriptor mismatch, caller-authority rejection, and production/destructive hard-disable; production remains open | Frozen |
