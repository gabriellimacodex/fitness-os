...
| Onboarding transition (disposable PG)     | `onboarding_transition` table + sink in `packages/database` (migration `0013_prd07_onboarding_transition`)                    | Synthetic onboarding transition evidence                                          | Disposable PG tests; append-only, dedup on exact-tuple repeat; API still in-memory `SyntheticOnboardingTransitionSink` | Active |
...
| Onboarding transition sink                | `OnboardingTransitionSink` + `SyntheticOnboardingTransitionSink` in `@fitness-os/domain`                                      | Domain onboarding ports                                                           | Unit tests; append-only; PG via `asOnboardingTransitionSink`                                                           | Active |
