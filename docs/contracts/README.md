...
| Retention preview (disposable PG)         | `privacy_retention_preview` table + repository in `packages/database` (migration `0013_prd21_privacy_retention_preview`)      | Synthetic retention preview persistence                                           | Disposable PG tests; keyed by `selectionDigest`; API write-through via `retentionPreviews` inject | Active |
