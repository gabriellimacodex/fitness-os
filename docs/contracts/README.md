...
| Onboarding readiness probe | `OnboardingReadinessProbe` + `SyntheticOnboardingReadinessProbe` in `@fitness-os/domain` | Domain onboarding ports | Unit tests; productionReady stays false under LEGAL_PRIVACY; PG-backed `schema` component via `createPostgresOnboardingReadinessProbe` in `packages/database` | Active |
...
| Onboarding claim-secret throttle | `ClaimFailureTracker` + `SyntheticClaimFailureTracker`, `evaluateClaimThrottle`, `checkClaimThrottle` in `@fitness-os/domain` | Domain onboarding ports; wired into `POST /v1/onboarding/invitations/inspect` and `POST /v1/onboarding/attempts` | Unit tests; process-local in-memory only, no PG-backed implementation yet | Active |
...
| Synthetic retention execution authorize | `privacySyntheticRetentionExecutionAuthorizeRequestSchema` / response in `privacy-governance.ts` | Synthetic API only | Disposable `allowSyntheticPrivacy` tests; the route still takes `previewExecuted`/`previewExpired`/`digestsMatch` directly from the request body | Frozen |
| Retention execution persisted-preview lookup | `resolveRetentionExecutionAuthorization` in `packages/domain` | Composes `authorizeRetentionExecution` with a real `PrivacyRetentionPreviewRecord` (executed/TTL/digest derived from the persisted row, never a caller-supplied boolean); caller supplies `previewTtlMs` explicitly, no default | Domain unit tests covering allow, missing/mismatched preview, executed, TTL boundary, and fail-closed malformed-input paths; not yet wired into the `apps/api` synthetic retention-execution-authorize route, which still trusts caller-supplied booleans | Active |
...
| Retention rule (disposable PG) | `privacy_retention_rule` table + `createPostgresPrivacyRetentionRuleRepository` in `packages/database` (migration `0018_prd21_privacy_retention_rule`) | Reuses the frozen `privacyRetentionRuleReferenceSchema` unmodified | Disposable PG integration tests; immutable per `ruleVersionId`, conflict on repeat; available for injection but not the default in server bootstrap | Active |
...
| Onboarding readiness probe (disposable PG) | `createPostgresOnboardingReadinessProbe` in `packages/database` | Wraps a base `OnboardingReadinessProbe` (defaults to `SyntheticOnboardingReadinessProbe`) and replaces only its `schema` component with `checkOnboardingSchemaReadiness`; recomputes `mechanismReady` | Disposable PG tests; optional inject via `PlatformOptions.onboarding.readinessProbe` (`apps/api`), not the default in server bootstrap; `productionReady` stays false | Active |
...
| Governance lifecycle binding verifier (disposable PG) | `createPostgresPrivacyGovernanceLifecycleBindingVerifier` in `packages/database` | `PrivacyGovernanceLifecycleBindingVerifier` (domain port); resolves a caller-presented binding against the real append-only ledger instead of an in-process seal | Disposable PG integration tests covering exact match, missing operation, mismatched proofId/outcome/requestId/processorId, and a sealed denied outcome; not yet composed into `apps/api`'s `governanceLifecycleVerifier` inject point, which still defaults to `SyntheticPrivacyGovernanceLifecycleBindingVerifier` | Active |
...
| Privacy schema readiness (disposable PG) | `createPostgresPrivacyReadinessProbe` in `packages/database`, wrapping `checkPrivacyCoreDatabaseReadiness` and `checkPrivacyGovernanceLifecycleDatabaseReadiness` | Overrides only the `migrations`/`repositories`/`governance_lifecycle` components of `PrivacyReadinessProbe`; every other component mirrors the base probe | Disposable PG integration tests; injectable via `PrivacySyntheticOptions.readiness` in `apps/api`, not the default in server bootstrap | Active |
...
| Retention preview (disposable PG) | `privacy_retention_preview` table + repository in `packages/database` (migration `0017_prd21_privacy_retention_preview`) | Synthetic retention preview persistence | Disposable PG tests; keyed by `selectionDigest`; API write-through via `retentionPreviews` inject, now composed by default into `createPrivacyPgPersistence`'s bundle | Active |
| Retention preview repository port | `PrivacyRetentionPreviewRepository` + `SyntheticPrivacyRetentionPreviewRepository` in `@fitness-os/domain` | Domain privacy-governance ports | Unit tests; PG via `createPostgresPrivacyRetentionPreviewRepository`; API write-through inject | Active |
