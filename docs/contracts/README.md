...
| Privacy schema readiness (disposable PG) | `createPostgresPrivacyReadinessProbe` in `packages/database`, wrapping `checkPrivacyCoreDatabaseReadiness` | Overrides only the `migrations`/`repositories` components of `PrivacyReadinessProbe`; every other component mirrors the base probe | Disposable PG integration tests; injectable via `PrivacySyntheticOptions.readiness` in `apps/api`, not the default in server bootstrap | Active |
| Onboarding transition (disposable PG) | `onboarding_transition` table + sink in `packages/database` (migration `0016_prd07_onboarding_transition`) | Synthetic onboarding transition evidence | Disposable PG tests; append-only, dedup on exact-tuple repeat; API still in-memory `SyntheticOnboardingTransitionSink` | Active |
...
| Retention preview (disposable PG) | `privacy_retention_preview` table + repository in `packages/database` (migration `0017_prd21_privacy_retention_preview`) | Synthetic retention preview persistence | Disposable PG tests; keyed by `selectionDigest`; API write-through via `retentionPreviews` inject | Active |
