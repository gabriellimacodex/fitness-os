...
describe('createApiClient', () => {
  ...
  it('resumes an attempt with a validated retry token and no-store', async () => {
    ...
    const response = await client.onboardingResumeAttempt(
    ...
    expect(fetch).toHaveBeenCalledWith(
      new URL(
        'https://api.example.com/platform/v1/onboarding/attempts/44444444-4444-4444-8444-444444444444/resume',
      ),
      {
  ...
  it('encodes the attempt identifier as one URL segment when resuming', async () => {
    const fetch = vi.fn<typeof globalThis.fetch>(async () =>
      Response.json({
        operation: {
          canonicalizationVersion: 'utf8-json-sha256.v1',
    ...
    await client.onboardingResumeAttempt('attempt/with-slash', 'b'.repeat(16));

    expect(fetch).toHaveBeenCalledWith(
      new URL(
        'https://api.example.com/v1/onboarding/attempts/attempt%2Fwith-slash/resume',
      ),
      expect.objectContaining({ method: 'POST' }),
    );
  });

  it('rejects an invalid retry token before resuming an attempt', async () => {
    const fetch = vi.fn<typeof globalThis.fetch>();
    const client = createApiClient({
      baseUrl: 'https://api.example.com',
      fetch,
    ...
    await expect(
      client.onboardingResumeAttempt(
        '44444444-4444-4444-8444-444444444444',
        'too-short',
      ),
    ).rejects.toThrow();
  ...
  it('throws a typed API error when resuming an attempt is unauthenticated', async () => {
    const fetch = vi.fn<typeof globalThis.fetch>(async () =>
      Response.json(
        {
          error: {
            code: 'UNAUTHENTICATED',
            message: 'Authentication required',
            requestId: 'req-resume-attempt-1',
          },
        },
        { status: 401 },
      ),
  ...
  it('does not echo raw content from a malformed resume-attempt payload', async () => {
