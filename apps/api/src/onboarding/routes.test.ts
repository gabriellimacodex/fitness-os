...
describe('policy-refresh and claim', () => {
  ...
  it('sets no-store on unexpected onboarding policy-refresh failures', async () => {
    const store = createOnboardingStore();
    seedIssuedInvitation(store, { claimSecret: CLAIM_SECRET });
    const { app } = buildSyntheticApp({ store });
    app.addHook('preHandler', async (request) => {
      if ((request.url.split('?')[0] ?? '').endsWith('/policy-refresh')) {
        throw new Error('private onboarding policy-refresh failure');
      }
    });
    ...
    const response = await app.inject({
      ...
      url: `/v1/onboarding/attempts/${attemptId}/policy-refresh`,
      payload: {
        retryToken: retryTokenSchema.parse('synthetic-retry-policy-failure'),
      },
    });
    const body = apiErrorResponseSchema.parse(response.json());
    ...
    expect(response.body).not.toContain(
      'private onboarding policy-refresh failure',
    );
    await app.close();
  });
