...
Follow the [Autonomous Delivery Control Plane](docs/execution/README.md). Standing authority applies only to a current `APPROVED` PRD. A `PROPOSED` roadmap entry never authorizes implementation. PRDs 00–02 are complete; PRD 03 is in progress on the Option A wave (Gate A package pending); PRD 21 is in progress on the Option A contract-freeze wave after the founder recorded that architecture decision; PRDs 04 and 07 are in progress; PRD 04 content publication is stopped on `HUMAN_PERCEPTION_REQUIRED`; PRDs 05–06, 08–20, and 22–24 remain approved under Autonomous Pilot V1 authorization; PRD 25 remains proposed and must not begin without separate explicit authorization.
