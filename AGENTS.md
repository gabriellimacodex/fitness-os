...
Follow the [Autonomous Delivery Control Plane](docs/execution/README.md). Standing authority applies only to a current `APPROVED` PRD. A `PROPOSED` roadmap entry never authorizes implementation. PRDs 00–02 are complete; PRD 03 is in progress on the isolated Option A wave; PRD 21 remains blocked pending Option A implementation resume after the founder recorded that architecture decision; PRDs 04 and 07 are in progress; PRDs 05–06, 08–20, and 22–24 remain approved under Autonomous Pilot V1 authorization; PRD 25 remains proposed and must not begin without separate explicit authorization.
